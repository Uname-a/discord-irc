...
class Bot {
  constructor(options) {
    ...
    // Cache lowercased versions for performance
    this.ircUsersLowerCase = this.ignoreUsers.irc.map(i => i.toLowerCase());
    this.discordUsersLowerCase = this.ignoreUsers.discord.map(i => i.toLowerCase());
    ...
    this.channelMapping = {};
    this.webhooks = {};
    this.webhookIds = new Set(); // Cache for webhook IDs
  ...
  connect() {
    ...
    // Cache webhook IDs in a Set for faster lookup
    this.webhookIds = new Set(Object.values(this.webhooks).map(wh => wh.id));
  ...
  ignoredIrcUser(user) {
    return this.ircUsersLowerCase.includes(user.toLowerCase());
  }

  ignoredDiscordUser(discordUser) {
    const ignoredName = this.discordUsersLowerCase.includes(
      discordUser.username.toLowerCase()
    );
    const ignoredId = this.ignoreUsers.discordIds.includes(discordUser.id);
  ...
  sendToIRC(message) {
    ...
    if (author.id === this.discord.user.id || this.webhookIds.has(author.id)) return;
    ...
    if (discordChannelName) {
      ...
      if (this.discord.channels.cache.has(discordChannelName)) {
        ...
        discordChannel = this.discord.channels.cache
          .find(c => c.type === 'text' && c.name === discordChannelName.slice(1));
      }
  ...
  getDiscordAvatar(nick, channel) {
    ...
    // Try to find exact matching case first, then case insensitive if not found
    let users = guildMembers.filter(findByNicknameOrUsername(true));
    if (users.size === 0) {
      users = guildMembers.filter(findByNicknameOrUsername(false));
    }

    // Return avatar only if there's exactly one matching user
